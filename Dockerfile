...
RUN cat > /crabclaw-data/.crabclaw/config.toml << 'EOF'
workspace_dir = "/crabclaw-data/workspace"
config_path = "/crabclaw-data/.crabclaw/config.toml"
api_key = ""
...
RUN chown -R 65534:65534 /crabclaw-data
...
COPY --from=builder /app/target/release/crabclaw /usr/local/bin/crabclaw

# Overwrite minimal config with DEV template (Ollama defaults)
COPY dev/config.template.toml /crabclaw-data/.crabclaw/config.toml
RUN chown 65534:65534 /crabclaw-data/.crabclaw/config.toml
