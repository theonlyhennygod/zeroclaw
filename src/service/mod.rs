...
const SERVICE_LABEL: &str = "com.crabclaw.daemon";
...
fn start(config: &Config) -> Result<()> {
    if cfg!(target_os = "macos") {
        ...
        run_checked(Command::new("systemctl").args(["--user", "start", "crabclaw.service"]))?;
        println!("✅ Service started");
        Ok(())
    } else {
        let _ = config;
...
fn stop(config: &Config) -> Result<()> {
    if cfg!(target_os = "macos") {
        ...
        let _ = run_checked(Command::new("systemctl").args(["--user", "stop", "crabclaw.service"]));
        println!("✅ Service stopped");
        Ok(())
    } else {
        let _ = config;
...
"#,
    ...
    println!("   Start with: crabclaw service start");
    Ok(())
}
...
fn install_linux(config: &Config) -> Result<()> {
    ...
    let exe = std::env::current_exe().context("Failed to resolve current executable")?;
    let unit = format!(
        "[Unit]\nDescription=CrabClaw daemon\nAfter=network.target\n\n[Service]\nType=simple\nExecStart={} daemon\nRestart=always\nRestartSec=3\n\n[Install]\nWantedBy=default.target\n",
        exe.display()
    );

    fs::write(&file, unit)?;
    let _ = run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]));
    let _ = run_checked(Command::new("systemctl").args(["--user", "enable", "crabclaw.service"]));
...
mod tests {
    ...
    fn linux_service_file_has_expected_suffix() {
        ...
        assert!(path.ends_with(".config/systemd/user/crabclaw.service"));
    }
}
