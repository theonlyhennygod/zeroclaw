...
// in `~/.crabclaw/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.crabclaw/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(crabclaw_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: crabclaw_dir.join(".secret_key"),
            enabled,
        }
    }
